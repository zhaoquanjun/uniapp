	...
	export default {
		...
		methods: {
			...
			_getAccountAndCardInfo() {
				get({
					...
					success: res => {
						...
						if (res.couponBalance && res.couponBalance > 0) {
							pay.card = res.couponBalance > this.rechargeValue ? this.rechargeValue : res.couponBalance
						}
						if (!res.couponBalance && res.expenseBalance > 0) {
							pay.account = res.expenseBalance > this.rechargeValue ? this.rechargeValue : res.expenseBalance
						}
						this.setData({
							accountInfo: {
								card: (res.couponBalance / 100).toFixed(2),
								account: (res.expenseBalance / 100).toFixed(2)
							},
							payInfo: pay
						})
			...
			_getPackagesList() {
				get({
					...
					success: res => {
						...
						const last = JSON.parse(JSON.stringify(res[0]))
						last.price = 0.01
						res.push(last)
						this.setData({
							packages: res,
			...
			_getWxPayParams() {
				...
				get({
					url: get_wx_pay_params + '?amount=' + this.total + '&body=微信支付&rechargeType=1',
					success: res => {
						console.log(res)
						this._payByWx(res)
					},
