		...
		<view :class="'user-dialog ' + ( showDialog ? 'user-dialog--show' : '' )">
			...
			<view class="user-dialog__container">
				...
				<scroll-view scroll-y nowrap style="height:540rpx">
					<view v-for="(item, index) in userList" :key="index" class="userList" @tap="switchUser" :data-item="item" :data-it="JSON.stringify(item)"
					 :data-index="index">
...
<script>
	...
	export default {
		...
		methods: {
			 ...
			 * @name 关闭用户选择弹层
			 */
			handleCloseUserDialogFun: function() {
				this.setData({
					showDialog: false
				});
			},
			 ...
			 * @name 发起用户切换
			 */
			handleLaunchUserSwitchFun: function() {
				if (!this.handleGetUserLoginStatusFun()) {
					app.globalData.login();
					return false;
				}
			...
			switchUser: function(e) {
				...
				// #ifdef H5
				const item = JSON.parse(e.currentTarget.dataset.it);
